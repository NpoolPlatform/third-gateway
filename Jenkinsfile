    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists third_gateway;"
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep third-gateway | grep $branch
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
          fi
          images=`docker images | grep entropypool | grep third-gateway | grep none | awk '{ print $3 }'`
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          if [ 0 -eq $rc -a x"$revlist" != x ]; then
            ...
            set +e
            docker images | grep third-gateway | grep $tag
            rc=$?
            set -e
            if [ 0 -eq $rc ]; then
              DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/third-gateway:latest/third-gateway:$branch/g" cmd/third-gateway/k8s/02-third-gateway.yaml
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/third-gateway/k8s/02-third-gateway.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/third-gateway/k8s/02-third-gateway.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
